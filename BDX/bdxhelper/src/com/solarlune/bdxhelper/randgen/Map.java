...
public class Map extends ArrayList<ArrayList<Cell>> {
    ...
    public Map(int sizeX, int sizeY, Scene scene) {
    ...
    public void generateMazeAlgo(int cellNum, int turnLeftWeight, int turnRightWeight, int straightWeight, String... genValues){

        if (genValues.length == 0)
        {
            System.out.println("ERROR - generateMazeAlgo needs a list of values to choose for generation");
            return;
        }
        ...
        for (int c = 0; c < cN; c++) {
            ...
            Vector2f l = BDXHMath.rotL(direction);
            Vector2f r = BDXHMath.rotR(direction);
            ...
            if (possibles.size() > 0) {
                ...
                Vector2f newPos = enforceWrapBehavior(pos.plus(direction));

                get(pos).connect(get(newPos), Random.choice(genValues));
                ...
                Cell nextOne = getRandomCell(Random.choice(genValues));

                while(nextOne.equals(get(pos)))
                    nextOne = getRandomCell(Random.choice(genValues));
    ...
    public Vector2f enforceWrapBehavior(Vector2f pos){
        if (wrapBehavior == Wrap.WRAP) {
            while (pos.y >= size())
                pos.y -= size();
            while (pos.y < 0)
                pos.y += size();
            while (pos.x >= get(0).size())
                pos.x -= size();
            while (pos.x < 0)
                pos.x += size();
        }
        else if (wrapBehavior == Wrap.CLAMP) {
            pos.y = Math.min(Math.max(pos.y, 0), size() - 1);
            pos.x = Math.min(Math.max(pos.x, 0), get(0).size() - 1);
        }
        return pos;
    }

    public void generateMazeAlgo(){
        generateMazeAlgo((sizeX * sizeY) / 2, 1, 1, 1, "x");
    }
    ...
    public boolean spaceEmpty(Vector2f pos) {
        ...
        return get(p).getValue().equals(EMPTY_CELL);
